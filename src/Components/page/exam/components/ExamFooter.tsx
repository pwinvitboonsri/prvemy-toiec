...
export function ExamFooter({
  ...
  useEffect(() => {
    if (audioSrc && audioRef.current && !hasPlayed) {
      if (!showAudioControls) { // Only auto-play in Sim mode (hidden controls)
        audioRef.current
          .play()
  ...
  }, [audioSrc, hasPlayed, showAudioControls]);
  ...
  // If showAudioControls is true, we might want to render a custom player or just standard controls.
  // For MVP/Practice, standard controls are fine? 
  // FSD says "Audio Control: Seek Bar, Full Controls".
    ...
    <footer className="h-16 md:h-20 bg-white border-t-2 border-[#111111] flex items-center justify-between px-3 md:px-8 shrink-0 z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] relative gap-2">
      ...
      {audioSrc && (
        <div className={cn("absolute left-1/2 -translate-x-1/2 top-2", !showAudioControls && "opacity-0 pointer-events-none")}>
          <audio
            ...
            className={cn("h-8", showAudioControls ? "block" : "hidden")}
          />
        </div>
      )}
      {!showAudioControls && audioSrc && (
        <audio
          ...
          onLoadedMetadata={handleLoadedMetadata}
          onEnded={handleEnded}
          className="hidden" // Hidden player for Sim mode
        />
      )}
